...
% Moreover, a line with less than m intersections with IntermediateSet should have m points.
    ...
    for i=1:size(L,1)
        ...
        elseif kI>m && kI<n
            ...
            %     SearchedLIndex=[SearchedLIndex;[i,n]];
            %     return; 
            % Else give this line higher priority to search, since it has had at least m points.
            % The acceleration is prominent, even better than bushier tree.
            % It is because the ignoraning of a searched line just saves very little resource.
            else
                ChasingSetIndex=RowIndex(1); ExcludedPIndex=[ExcludedPIndex,RowIndex(1)]; TrivialorNot=1;
                return;
            end   
        ...
        elseif kI<m           
            ...
            if kI+kR==m
                ...
                SearchedLIndex=[SearchedLIndex;[i,m]];
                return;
            % Bushier tree costs more, just promote the priority is faster.
            elseif kI+kR-1==m
                ChasingSetIndex=RowIndex(1); ExcludedPIndex=[ExcludedPIndex,RowIndex(1)]; TrivialorNot=1;
                return;
            end
