...
%% Roughly removing automorphism. 2025.11.5
...
% Then we can directly throughly search the second line passing through e1.
IntermediateSet=[IntermediateSet;cell2mat(L(LinesPassinge1(1,1),[2:LinesPassinge1(1,2)])')];
ExcludingIndex=[ExcludingIndex;find(ismember(P,cell2mat(L(LinesPassinge1(1,1),:)'),'rows'))]; 
SearchedLIndex=[SearchedLIndex;LinesPassinge1(1,1),LinesPassinge1(1,2)]; LinesPassinge1(1,:)=[];
RemainingSet(ExcludingIndex',:)=[];
%ElementwiseGen(IntermediateSet,RemainingSet,SearchedLIndex,LinesPassinge1);
...
%% RemainingSets are the points to be added to the type(m,n) set. IntermediateSet is the generating type(m,n) set in process.
%% SearchedLIndex is returned by the PossiblemnSet to record the searched lines. RoughlyNoAutoIndex is to record the intersection of the initial line.
function ElementwiseGen(IntermediateSet,RemainingSet,SearchedLIndex,RoughlyNoAutoIndex)
% K is the size of TypemnSet in the final.
global AllTypemnSet; global K; global L;
...
elseif all(size(IntermediateSet,1)+size(RemainingSet,1)>=K) && all(~isempty(RemainingSet))
    ...
    [Consistent,ChasingSetIndex,ExcludingIndex,SearchedLIndex]=PossiblemnSet(IntermediateSet,RemainingSet,SearchedLIndex,RoughlyNoAutoIndex);
    ...
    if Consistent
        ...
        if isempty(ChasingSetIndex) && isempty(ExcludingIndex)
            % If the assigned Roughly noAuto is not fulfilled, then search
            % the lines priorly.
            % RoughlyNoAutoIndex(find(ismember(RoughlyNoAutoIndex(:,1),SearchedLIndex(:,1))),:)=[];
            % if ~isempty(RoughlyNoAutoIndex)
            %     % tIndex is the index of folking point. t is the adding point.
            %     tIndex=find(ismember(RemainingSet,cell2mat(L(RoughlyNoAutoIndex(1,1),:)'),'rows'),1);
            %     t=RemainingSet(tIndex,:); RemainingSet(tIndex,:)=[];
            %     ElementwiseGen([IntermediateSet;t],RemainingSet,SearchedLIndex,RoughlyNoAutoIndex);
            %     ElementwiseGen(IntermediateSet,RemainingSet,SearchedLIndex,RoughlyNoAutoIndex);
            % else
            %     ElementwiseGen([IntermediateSet;RemainingSet(1,:)],RemainingSet(2:size(RemainingSet,1),:),SearchedLIndex,RoughlyNoAutoIndex);
            %     ElementwiseGen(IntermediateSet,RemainingSet(2:size(RemainingSet,1),:),SearchedLIndex,RoughlyNoAutoIndex); 
            % end  
            ElementwiseGen([IntermediateSet;RemainingSet(1,:)],RemainingSet(2:size(RemainingSet,1),:),SearchedLIndex,RoughlyNoAutoIndex);
            ElementwiseGen(IntermediateSet,RemainingSet(2:size(RemainingSet,1),:),SearchedLIndex,RoughlyNoAutoIndex); 
        ...
        elseif ~isempty(ChasingSetIndex)
            % If the excludingIndex is empty, then it means we have exact
            ...
            if isempty(ExcludingIndex) 
                ...
                ElementwiseGen([IntermediateSet;ChasingSet],RemainingSet,SearchedLIndex,RoughlyNoAutoIndex);
            ...
            % If condition permitted, use parfor. Parallel is powerful.
                ...
                for i=1:size(ChasingSetIndex,1)
                    ...
                    ElementwiseGen([IntermediateSet;ChasingSet],TempRemainingSet,SearchedLIndex,RoughlyNoAutoIndex);                 
                end  
            end
