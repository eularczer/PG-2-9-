...
% RoughlyNoAutoIndex is to record the intersection of the initial line.
function[Consistent,ChasingSetIndex,ExcludingIndex,SearchedLIndex]=PossiblemnSet(IntermediateSet,RemainingSet,SearchedLIndex,RoughlyNoAutoIndex)
...
for i=1:size(L,1)
    ...
    if  ~isempty(SearchedOrNot)
        ...
        % If the searched tree not belongs to RoughlyNoAutoIndex, then
        % return inconsistent.
        if ~isempty(RoughlyNoAutoIndex)
            FulfilNoAutoOrNot=find(ismember(RoughlyNoAutoIndex(:,1),i),1);
            if ~isempty(FulfilNoAutoOrNot) 
                if ~(RoughlyNoAutoIndex(FulfilNoAutoOrNot,2)==kI) Consistent=0; return; end 
            end
        end
        continue;
    end
