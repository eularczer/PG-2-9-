% The function is to generate 2-dim subspace when input two vectors, and
% returns back a q^2 array (all the vectors in 2-dim nonprojective space).
% If not overly generating points, then some points will escape being removed.
% For example, TwoDimSubspace(001,010) can contain 021 which is not standard in
% P, but should be changed to standard 012.
function [w]=TwoDimSubspace(u,v)
    w=[];
    % The configurations.
    global p; global h; global q; global D;
    % Depending on the vector space dimension, span the 2-dim vector subspace, 
    % The 2-dim subspace spanned by the representative u depends on the specific GF(q).
    if p==2 && h==1
        for i=0:p^h-1
            for j=0:p^h-1
                for k=1:D
                    % Vec2d means the generated vectors in 2d space.
                    Vec2d(k)=PosAdditionGF2(PosMultiplicationGF2(i,u(k)),PosMultiplicationGF2(j,v(k)));
                end
                w=[w;Vec2d];
            end
        end
    elseif p==2 && h==2
        for i=0:p^h-1
            for j=0:p^h-1
                for k=1:D
                    % Vec2d means the generated vectors in 2d space.
                    Vec2d(k)=PosAdditionGF4(PosMultiplicationGF4(i,u(k)),PosMultiplicationGF4(j,v(k)));
                end
                w=[w;Vec2d];
            end
        end
    elseif p==2 && h==3
        for i=0:p^h-1
            for j=0:p^h-1
                for k=1:D
                    % Vec2d means the generated vectors in 2d space.
                    Vec2d(k)=PosAdditionGF8(PosMultiplicationGF8(i,u(k)),PosMultiplicationGF8(j,v(k)));
                end
                w=[w;Vec2d];
            end
        end
    elseif p==3 && h==1
        for i=0:p^h-1
            for j=0:p^h-1
                for k=1:D
                    % Vec2d means the generated vectors in 2d space.
                    Vec2d(k)=PosAdditionGF3(PosMultiplicationGF3(i,u(k)),PosMultiplicationGF3(j,v(k)));
                end
                w=[w;Vec2d];
            end
        end
    elseif p==3 && h==2
        for i=0:p^h-1
            for j=0:p^h-1
                for k=1:D
                    % Vec2d means the generated vectors in 2d space.
                    Vec2d(k)=PosAdditionGF9(PosMultiplicationGF9(i,u(k)),PosMultiplicationGF9(j,v(k)));
                end
                w=[w;Vec2d];
            end
        end
