...
global P; global L; global D; global p; global h; global NumP; global q;
...
L_NumVer=q+1; L={}; r=1; NumVer=NumP;
for i=1:NumVer
...
if AG==2
    ...
    RemovePos=[];
    for i=1:size(L,1)
        if ~isempty(find(ismember(cell2mat(L(i,:)'),x,'rows')))
...
%% Change the lines of cell format to 3D matrix format.
NumL=size(L,1);
L3D = permute(reshape(cell2mat(L), NumL, D, L_NumVer), [1, 3, 2]);
1;